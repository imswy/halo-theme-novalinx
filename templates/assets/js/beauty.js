...
(function () {
  ...
  const EvanBigBanner = function (options) {
    if (!(this instanceof EvanBigBanner)) {
      return new EvanBigBanner(options);
    }
  ...
  EvanBigBanner.prototype = {
    ...
    fnSetStyle() {
      ...
      if (this.options.titleTiktok) {
        this.dom.bannerTitle.addClass("evan-tiktok-text");
      }
      ...
      if (this.options.followMode) {
        if (!this.options.wavesColor) {
          this.dom.waves.css("fill", "var(--background)");
        }
      }
      if (this.options.followTheme) {
        if (!this.options.wavesColor) {
          this.dom.waves.css("fill", "var(--theme)");
        }
        if (!this.options.titleTiktok) {
          this.dom.bannerTitle.css("color", "var(--theme)");
        }
      }
    },
    fnPrintText() {
      ...
      let _textContent = "",
        _outTextContent = "";
      if (_this.options.titleText) {
        _textContent = _this.options.titleText;
      } else {
        _textContent = _this.dom.bannerTitle.text();
      }
      let i = 0;
      ...
      const _lastPrintText = () => {
        setTimeout(() => {
          if (_isShowLast) {
            $("#EvanBigBannerLastTitleTip").css({ visibility: "hidden" });
          } else {
            $("#EvanBigBannerLastTitleTip").css({ visibility: "visible" });
          }
    ...
    init() {
      ...
      $(function () {
        ...
        _this.fnSetStyle();
        if (_this.options.titleText) {
          _this.dom.bannerTitle.text(_this.options.titleText);
        } else {
          _this.dom.bannerSubTitle.css("opacity", 1);
        }
      });
    },
  };
